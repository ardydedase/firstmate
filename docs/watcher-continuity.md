...
A branch drain or acknowledgement rebuilds a lost or corrupt `state/.branch-eligible-rows` from that record's list under the queue lock before presenting, acknowledging, or reclaiming, keeping a snapshot lost to descendant-process churn from wedging the queue.
